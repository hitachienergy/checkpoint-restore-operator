...
func handleLivenessProbe(config *util.ConfigMapEntry) {
	...
	for {
		...
		for _, pod := range config.Pods {
			...
			util.DeletePod(kClient, context.Background(), pod.KRef)
			util.GetDeletedPodsChannelFor(config.Id) <- *pod
			util.CreatePod(kClient, context.Background(), config.LabelSelector, pod.KRef)
		}
	}
}
